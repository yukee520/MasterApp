...
import { CommonActions, findFocusedRoute, getActionFromState, getPathFromState, getStateFromPath, NavigationContainerRefContext, NavigationHelpersContext, NavigationRouteContext, useStateForPath } from '@react-navigation/core';
...
export const useBuildAction = () => {
  ...
  const buildAction = React.useCallback(href => {
    ...
    if (state) {
      const action = getActionFromStateHelper(state, options?.config) ?? CommonActions.reset(state);
      return {
  ...
  }, [navigation, options?.config, getStateFromPathHelper, getActionFromStateHelper]);
  return buildAction;
};
